...
module mul8x8comb(
   ...
   logic [3:0]    ShiftPPLeft;
   logic [3:0]    NegPP;
   logic [3:0]    ClrPP;
   
   logic [15:0]   PartialProduct[4:0];		// Was 3
   
   logic [7+1:0]  PX;		// +X
   logic [7+1:0]  MX;		// -X
   
   // Carry save adders (CSA) signals
   logic [15:0]   CSA_AIn[4:0];
   logic [15:0]   CSA_BIn[4:0];
   logic [15:0]   CSA_CarryIn[4:0];
   logic [15:0]   CSA_SumOut[4:0];
   logic [15:0]   CSA_CarryOut[4:0];
   
   // Was added for compatibility with Synopsys DC
   logic [6:0]    PX_H_Vector;		// 15..9
   logic [6:0]    MX_H_Vector;		// 15..9
   // Was added for compatibility with Synopsys DC	
